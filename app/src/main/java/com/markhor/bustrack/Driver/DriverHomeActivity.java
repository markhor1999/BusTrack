...
public class DriverHomeActivity extends AppCompatActivity {
    ...
    //
    //private ArrayList<BusLocations> mBusLocationsList = new ArrayList<>();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_driver_home);
        Objects.requireNonNull(getSupportActionBar()).setTitle("Driver");
        //
    ...
    //Start the location service in background if it is not already running
    private void startLocationService() {
        if(!isLocationServiceRunning()) {
            Log.d(TAG, "startLocationService: Location Service is Started");
            Intent serviceIntent = new Intent(this, LocationServices.class);
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                DriverHomeActivity.this.startForegroundService(serviceIntent);
            }
            else {
